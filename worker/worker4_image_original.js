...
async function processOriginalImages() {
    // console.log('[Worker 4] Buscando notas sin imagen original...');
    ...
    if (error) {
        console.error('[Worker 4] DB Error:', error.message);
        return;
    }
    ...
    for (const article of articles) {
        try {
            console.log(`[Worker 4] Extrayendo og:image de: ${article.source_url}`);
            ...
            if (!ogImage) {
                console.log(`[Worker 4] No se encontró og:image. Marcando como NO_IMAGE`);
            ...
            // 2. Download Image
            console.log(`[Worker 4] Descargando imagen original: ${ogImage}`);
            ...
            console.error(`[Worker 4] Error en artículo ${article.id}:`, err.message);
...
async function startWorker() {
    console.log('🚀 Iniciando Worker 4 (Original Scraper Images)');
    while (true) {
        await processOriginalImages();
        await new Promise(r => setTimeout(r, 60000)); // Correr cada minuto
    }
}
